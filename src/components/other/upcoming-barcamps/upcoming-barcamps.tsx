...
export class UpcomingBarcamps {
  ...
  render() {
    return (
      <Host>
        <stellar-layout
          class="bb border-theme-2 dm-border-theme-7"
          padding="large"
        >
          ...
          {this.conferences &&
            this.conferences.map((conf) => (
              <div class="bt bb pa2 border-theme-5 flex flex-column flex-row-m flex-row-l space-between items-center">
                <div class="w-100">
                  <h4 class="parco i">{conf.name}</h4>
                  <p>{conf.venue.address}</p>
                  <p>
                    {conf.start.format("MMMM DD, h:mma")} to{" "}
                    {conf.end.format("h:mma")}
                  </p>
                </div>
                <stellar-grid
