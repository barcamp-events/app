...
export class BarcampHostNameYourEvent {
  ...
  async saveStep(e) {
    ...
    if (!this.conference) {
      ...
      this.history.push(`/host/${conference.key}/budget`, { conferenceId: conference.key, conference });
    } else {
    ...
    return <Host>
      <midwest-layout type="supporting-content" class="align-start" size="large">
              ...
              <midwest-form ajax onSubmitted={this.saveStep.bind(this)}>
                <midwest-grid cols="1" noresponsive>
                  ...
                  <midwest-input
                    ...
                    onUpdate={(e) => { this.mantra = e.detail; }}
                  />
                  <midwest-grid>
                    <midwest-input-date 
                      ...
                      label="Start"
                      onUpdate={(e) => { this.start = e.detail; }}
                    />
                    <midwest-input-date 
                      name="conference[end]"
                      type="datetime"
                  ...
                  <midwest-grid>
                    <midwest-select
                      ...
                      onUpdate={(e) => { this.color = e.detail; }}
                    >
                      {this.colors.map(color => <midwest-item value={color} checked={this.conference?.color === color}>{color}</midwest-item>)}
                    </midwest-select>
                    ...
                    <midwest-switch
                      ...
                      onUpdate={(e) => { this.dark = e.detail.checked; }}
                    >
                      <p slot="yes">Dark Mode Enabled</p>
                      <p slot="no">Dark Mode Disabled</p>
                    </midwest-switch>
                  </midwest-grid>
                  <p class="text-sm text-center m-auto">By hosting an event on BarCamp Events, you agree to follow the <a>Code of Conduct</a>, <a>Privacy Policy</a>, and <a>Terms of Service</a>. </p>
        ...
        <aside class="sticky top-0 bottom-0 m-auto mt-0">
          <barcamp-host-event-preview conference={this.conference} color={this.color} dark={this.dark} name={this.name} mantra={this.mantra} />
        </aside>
      </midwest-layout>
    </Host>
  }
