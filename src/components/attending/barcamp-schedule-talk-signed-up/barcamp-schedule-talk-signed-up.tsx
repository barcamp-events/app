      ...
      <stellar-card>
        ...
        <footer>
          <stellar-grid style={{"--grid-width": "auto"}} class="justify-between">
            ...
            {Notifications.supported && !this.talk.isRecievingNotification(this.user) && (this.user && this.talk.speakerKey !== this.user.key) && <stellar-button tag="button" onClick={this.onRemindMe.bind(this)} ghost dark><stellar-asset name="notifications-outline" />Reminder me</stellar-button>}
            {Notifications.supported && !this.talk.isRecievingNotification(this.user) && this.notifPermission !== "denied" && <stellar-button tag="button" onClick={this.onRemindMe.bind(this)} ghost dark><stellar-asset name="notifications-off" /><stellar-tooltip>Reminder off</stellar-tooltip></stellar-button>}
            {Notifications.supported && this.talk.isRecievingNotification(this.user) && this.notifPermission !== "denied" && <stellar-button tag="button" onClick={this.onDontRemindMe.bind(this)} ghost dark><stellar-asset name="notifications" /><stellar-tooltip>Reminder on</stellar-tooltip></stellar-button>}
