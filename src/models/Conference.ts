...
await Conference.create({
	name: "XYZ",
	site_link: "https://barcampomaha.org",
	ticket_link: "https://opencollective.com/barcamp-omaha/events/barcamp-omaha-2022-location-tbd-6df56ef2/order/41550",
	color: "green",
	dark: true,
	year: "2022",
	slug: "xyz",
	...
	approved: true,
	createable: true,
	start: "Sat, 20 Aug 2019 14:00:00 GMT",
	end: "Sat, 20 Aug 2019 22:00:00 GMT"
})
*/
export default class Conference extends FirebaseModel {
  ...
  @prop({
    defaultValue: "Share Your Passion",
    emptyValue: "Share Your Passion",
  })
  ...
  async unattend(user: User) {
    if (this.is_user_attending(user)) {
      this.attendees = this.attendees.filter((key) => {
        return user.key !== key;
      });
      return await this.save();
    }
  ...
  async removeSponsor(sponsor: Sponsor) {
    if (this.is_sponsoring(sponsor)) {
      this.sponsors = this.sponsors.filter((key) => {
        return sponsor.key !== key;
      });
      return await this.save();
    }
  }
  ...
  async removeLineItem(lineItem: LineItem) {
    this.lineItems = this.lineItems.filter((key) => {
  ...
  async getTalksInOrder() {
    const talks = (await Talk.where(
      ["conferenceKey", "==", this.key],
      "many"
    )) as Talk[];
  ...
  async createTracks(
    tracks: string[] = ["Technology", "Design", "Makers", "Kitchen Sink"]
  ): Promise<Boolean> {
    let trackKeys = [];
    ...
    await asyncForEach(tracks, async (name) => {
      const track = await Track.create(
        new Track({ name, conferenceKey: this.key, talks: talks })
      );
  ...
  public isManagedBy(user: User): boolean {
  ...
  static async list(): Promise<Conference[]> {
    return await super.list();
  }
  static async create(data: Conference): Promise<Conference> {
    return await super.create(data);
  }
  static async update(data: Conference): Promise<Conference> {
  ...
  static async upcoming() {
    let conferences = [];
    let today = Dayjs();
    let result = await Conference.list();

    result.forEach((conference) => {
      if (today.isBefore(conference.start)) {
