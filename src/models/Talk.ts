...
export default class Talk extends FirebaseModel {
  ...
  get friendlyLength() {
    return `From ${this.time.format(Talk.format)} to ${this.time
      .add(this.talkLength, "minute")
      .format(Talk.format)}`;
  }
  ...
  async dontSendNotification(user: User) {
    if (this.isRecievingNotification(user)) {
      this.notify = this.notify.filter((key) => {
        return user.key !== key;
      });
      return await this.save();
    }
  ...
  static async release(target: Talk) {
    target.signingUpKey = null;
    target.claimedAt = null;

    await target.save();
  }

  static async reset(target: Talk) {
    await target.populate({
      speakerKey: undefined,
      claimedAt: undefined,
      signingUpKey: undefined,
      title: undefined,
